...
in the selected index pattern and the document `discover:defaultFields` properties. You can 
<<adding-columns, add fields to the Documents table>> from the Fields list. You can
<<sorting, sort the listed documents>> by any indexed field that's included in the table.
